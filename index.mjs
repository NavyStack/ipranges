    ...
    for (const sourceFilePath of sourceFilePaths) {
      try {
        ...
        const outputFileName = path.join(
          ...
          `${path.basename(sourceFilePath, '.txt')}${outputSuffix}.txt`
        )
        ...
        console.log(
          `File "${outputFileName}" created with merged CIDR addresses.`
        )
      } catch (error) {
        console.error(
          `Error processing file "${sourceFilePath}": ${error.message}`
        )
      }
    }
